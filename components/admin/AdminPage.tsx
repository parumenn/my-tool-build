...
const AdminPage: React.FC = () => {
  ...
  const chartData = useMemo(() => {
    ...
    return Object.entries(hours).map(([name, アクセス数]) => ({ name, アクセス数 }));
  }, [stats]);
        ...
        {activeTab === 'dashboard' && stats && (
          <div className="space-y-6 animate-fade-in">
             ...
             <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700">
                   <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">累計アクセス数</p>
                   <p className="text-3xl font-black text-indigo-600">{stats.total_pv.toLocaleString()}</p>
                </div>
                <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700">
                   <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">受信メッセージ</p>
                   <p className="text-3xl font-black text-pink-600">{messages.length.toLocaleString()}</p>
                </div>
                <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700">
                   <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">遮断済みIP数</p>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700">
                   <div className="flex justify-between items-center mb-6">
                      <h3 className="font-black flex items-center gap-2 text-gray-700 dark:text-gray-200"><TrendingUp size={18} className="text-blue-500" /> 24時間のアクセス推移</h3>
                      <span className="text-[10px] font-bold bg-blue-50 dark:bg-blue-900/30 text-blue-600 px-2 py-1 rounded">リアルタイム更新</span>
                   </div>
                   <div className="h-64 w-full">
                      <ResponsiveContainer width="100%" height="100%">
                         <AreaChart data={chartData}>
                            <defs>
                               <linearGradient id="colorアクセス数" x1="0" y1="0" x2="0" y2="1">
                            ...
                            />
                            <Area type="monotone" dataKey="アクセス数" name="アクセス数" stroke="#3b82f6" strokeWidth={3} fillOpacity={1} fill="url(#colorアクセス数)" />
                         </AreaChart>
                      </ResponsiveContainer>
                   </div>
                </div>
                      ...
                      {rankingData.map((item, idx) => (
                         <div key={idx} className="space-y-1">
                            <div className="flex justify-between text-xs font-bold">
                               ...
                               <span className="text-indigo-600">{item.count} <span className="text-[10px] text-gray-400">件</span></span>
                            </div>
           ...
           <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden animate-fade-in">
              <div className="p-6 border-b dark:border-gray-700 flex justify-between items-center">
                 <div>
                    ...
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">システム全体の監視</p>
                 </div>
              ...
              <div className="overflow-x-auto">
                 <table className="w-full text-sm text-left">
                    <thead className="bg-gray-50 dark:bg-gray-900/50 text-[10px] uppercase font-black text-gray-400 tracking-wider">
                       <tr>
                          <th className="px-6 py-4">日時 (年月日 時:分:秒.ミリ秒)</th>
                          <th className="px-6 py-4">リクエスト内容</th>
                          <th className="px-6 py-4">IPアドレス</th>
                          <th className="px-6 py-4">応答速度(ミリ秒)</th>
                          <th className="px-6 py-4">閲覧環境 (ブラウザ情報)</th>
                       </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50 dark:divide-gray-700">
        ...
        {activeTab === 'security' && (
          <div className="space-y-6 animate-fade-in">
             <div className="bg-red-50 dark:bg-red-900/20 p-8 rounded-3xl border border-red-100 dark:border-red-800 flex items-center gap-6">
                ...
                <div>
                   <h3 className="font-black text-xl text-red-800 dark:text-red-400">攻撃自動保護システム：稼働中</h3>
                   <p className="text-sm text-red-700 dark:text-red-300 mt-1 leading-relaxed">
                      1分間に60回以上のアクセスがあったIPアドレスを自動的に検出し、サイト全体へのアクセスを拒否します。<br/>
                      遮断実行時は、設定されたアラート通知先へ即座にメールが送信されます。
                   </p>
                </div>
             </div>
                ...
                <div className="overflow-x-auto">
                   <table className="w-full text-sm text-left">
                      <thead className="bg-gray-50 dark:bg-gray-900/50 text-[10px] uppercase font-black text-gray-400">
                         <tr><th className="px-6 py-4">IPアドレス</th><th className="px-6 py-4">遮断された理由</th><th className="px-6 py-4">遮断時刻</th><th className="px-6 py-4">操作</th></tr>
                      </thead>
                      <tbody className="divide-y divide-gray-50 dark:divide-gray-700">
