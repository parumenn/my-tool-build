  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-20">
      <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-2xl w-full max-w-md mx-auto">
        <button onClick={() => setMode('encode')} className={`flex-1 py-3 rounded-xl text-sm font-black transition-all ${mode === 'encode' ? 'bg-white dark:bg-gray-700 text-orange-600 dark:text-orange-400 shadow-md' : 'text-gray-500'}`}>送信 (File ➔ B64)</button>
        <button onClick={() => setMode('decode')} className={`flex-1 py-3 rounded-xl text-sm font-black transition-all ${mode === 'decode' ? 'bg-white dark:bg-gray-700 text-orange-600 dark:text-orange-400 shadow-md' : 'text-gray-500'}`}>受信 (B64 ➔ File)</button>
      </div>
      ...
      {mode === 'encode' ? (
        <div className="space-y-4">
          <div className="bg-white dark:bg-dark-lighter rounded-3xl p-6 md:p-10 shadow-sm border border-gray-100 dark:border-gray-700">
            ...
            <p className="text-xs text-gray-500 mb-6">スマホの画像やドキュメントを共有用のテキストコードに変換します。</p>
          ...
          {base64 && (
            <div className="bg-white dark:bg-dark-lighter rounded-3xl p-6 shadow-xl border border-orange-100 dark:border-orange-900/30 animate-fade-in">
              ...
              <textarea readOnly value={base64} className="w-full h-48 md:h-64 p-4 text-[16px] font-mono bg-slate-900 text-green-400 border border-slate-700 rounded-2xl focus:outline-none scroll-smooth" />
            </div>
          )}
        </div>
      ) : (
        <div className="bg-white dark:bg-dark-lighter rounded-3xl p-6 md:p-10 shadow-sm border border-gray-100 dark:border-gray-700 animate-fade-in space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea value={inputBase64} onChange={(e) => setInputBase64(e.target.value)} placeholder="data:image/png;base64,..." className="w-full h-48 p-4 text-[16px] font-mono border-2 border-gray-100 dark:border-slate-700 rounded-2xl focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 bg-gray-50 dark:bg-slate-800 dark:text-white" />
              </div>
              ...
              <button onClick={handleDecode} disabled={!inputBase64} className="w-full py-5 bg-orange-600 text-white font-black rounded-2xl hover:bg-orange-700 disabled:opacity-30 flex items-center justify-center gap-2 shadow-xl shadow-orange-200 dark:shadow-none transition-all active:scale-95">
