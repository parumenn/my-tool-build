...
const AdminPage: React.FC = () => {
  ...
  const removeDosPattern = (index: number) => {
     if (config.dos_patterns.length <= 1) return;
     const newPatterns = config.dos_patterns.filter((_, i) => i !== index);
     setConfig({ ...config, dos_patterns: newPatterns });
     setIsDirty(true);
  };

  const updateDosPattern = (index: number, field: keyof DosPattern, value: number) => {
     const newPatterns = [...config.dos_patterns];
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-10 text-center">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button type="submit" disabled={isLoggingIn} className="w-full py-4 bg-gray-900 dark:bg-white text-white dark:text-gray-900 font-black rounded-2xl flex items-center justify-center gap-2 hover:opacity-90 active:scale-95">
        ...
        <div className="flex bg-gray-100 dark:bg-gray-700 p-1 rounded-xl overflow-x-auto no-scrollbar max-w-[50%]">
           {['dashboard', 'logs', 'messages', 'security', 'settings'].map(t => (
             <button key={t} onClick={() => setActiveTab(t as any)} className={`px-4 py-1.5 rounded-lg text-xs font-black transition-all whitespace-nowrap ${activeTab === t ? 'bg-white dark:bg-gray-600 text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                {t === 'dashboard' ? '統計' : t === 'logs' ? 'ログ' : t === 'messages' ? '受信箱' : t === 'security' ? '保護状態' : 'SMTP設定'}
             </button>
           ))}
        </div>
      ...
      <main className="max-w-7xl mx-auto p-6 md:p-10">
        {activeTab === 'dashboard' && (
           <div className="space-y-6 animate-fade-in">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                 <div className="lg:col-span-2 bg-white dark:bg-dark-lighter p-8 rounded-3xl border dark:border-gray-700">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                 ...
                 <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl border dark:border-gray-700 shadow-sm space-y-6">
                    <div className="flex justify-between items-center">
                       <h3 className="font-black text-xl flex items-center gap-3"><Shield className="text-red-500" /> DOS保護ルール設定</h3>
                       <button onClick={addDosPattern} className="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-colors" title="パターンを追加"><Plus size={20} /></button>
                    </div>
                    
                    <div className="space-y-4">
                       {config.dos_patterns.map((p, idx) => (
                          <div key={idx} className="p-5 bg-gray-50 dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 space-y-4 relative group">
                             <div className="grid grid-cols-2 gap-4">
                                <div>
                                   <label className="text-[10px] font-black text-gray-400 uppercase">アクセス回数</label>
                                   <input type="number" value={p.count} onChange={e => updateDosPattern(idx, 'count', Number(e.target.value))} className="w-full p-2 bg-white dark:bg-gray-800 border rounded-lg font-bold" />
                                </div>
                                <div>
                                   <label className="text-[10px] font-black text-gray-400 uppercase">判定秒数</label>
                                   <input type="number" value={p.seconds} onChange={e => updateDosPattern(idx, 'seconds', Number(e.target.value))} className="w-full p-2 bg-white dark:bg-gray-800 border rounded-lg font-bold" />
                                </div>
                             </div>
                             <div>
                                <label className="text-[10px] font-black text-gray-400 uppercase flex items-center gap-1">
                                   遮断時間 (分) {p.block_minutes === 0 && <span className="text-red-500 flex items-center gap-1"><Infinity size={10} /> 永久遮断</span>}
                                </label>
                                <div className="flex gap-2 items-center">
                                   <input type="number" value={p.block_minutes} onChange={e => updateDosPattern(idx, 'block_minutes', Number(e.target.value))} className="flex-1 p-2 bg-white dark:bg-gray-800 border rounded-lg font-bold" placeholder="0で無制限" />
                                   <button onClick={() => updateDosPattern(idx, 'block_minutes', 0)} className={`p-2 rounded-lg border transition-colors ${p.block_minutes === 0 ? 'bg-red-500 text-white' : 'text-gray-400'}`} title="永久遮断に設定"><Infinity size={18} /></button>
                                </div>
                                <p className="text-[9px] text-gray-400 mt-1">※0分を指定すると自動解除されない永久遮断になります</p>
                             </div>
                             
                             {config.dos_patterns.length > 1 && (
                                <button onClick={() => removeDosPattern(idx)} className="absolute -top-2 -right-2 bg-white dark:bg-gray-700 text-red-500 p-1.5 rounded-full shadow-md border opacity-0 group-hover:opacity-100 transition-opacity"><Minus size={16} /></button>
                             )}
                          </div>
                       ))}
                    </div>
                    ...
                    <div className="pt-4 space-y-4 border-t dark:border-gray-700">
                       <label className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-900 rounded-xl cursor-pointer">
                          ...
                          <span className="font-bold text-sm">遮断実行時にメールで通知する</span>
                       </label>
                       
                       <button onClick={() => handleUpdateSettings()} className="w-full py-4 bg-blue-600 text-white font-black rounded-2xl shadow-xl hover:bg-blue-700 transition-all flex items-center justify-center gap-2 active:scale-95">
                          {configMsg ? <CheckCircle2 /> : <Settings />} {configMsg || '保護ルールを保存'}
                       </button>
                    </div>
                 </div>
                 ...
                 <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border dark:border-gray-700 overflow-hidden">
                    <div className="p-6 border-b dark:border-gray-700 flex justify-between items-center">
                       <h3 className="font-black flex items-center gap-2"><XCircle className="text-red-500" /> 現在遮断中のIPアドレス</h3>
                       <button onClick={() => fetchData(false)} className="p-2 text-gray-400 hover:text-blue-500"><RefreshCw size={18} /></button>
                    </div>
                    <div className="overflow-x-auto">
                       <table className="w-full text-sm text-left">
                          <thead className="bg-gray-100 dark:bg-gray-900/50 text-[10px] uppercase font-black text-gray-400 tracking-wider">
                             <tr><th className="px-6 py-4">IP</th><th className="px-6 py-4">理由</th><th className="px-6 py-4">残り時間</th><th className="px-6 py-4">操作</th></tr>
                          </thead>
                          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                             {Object.keys(blockedIps).length === 0 ? (
                                <tr><td colSpan={4} className="px-6 py-12 text-center text-gray-400 font-bold">遮断中のIPはありません</td></tr>
                                ...
                                return (
                                   <tr key={ip} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                      ...
                                      <td className="px-6 py-4">
                                         {isPerm ? (
                                            <span className="bg-red-100 text-red-700 px-2 py-0.5 rounded text-[10px] font-black flex items-center gap-1 w-fit"><Infinity size={10} /> 永久</span>
                                         ) : (
                                            <span className="text-orange-500 font-bold">{timeLeft} 分</span>
                                         )}
                                      </td>
                                      <td className="px-6 py-4">
                                         <button onClick={async () => { if(confirm('遮断を解除しますか？')) { await fetch(`./backend/admin_api.php?action=unblock_ip`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Admin-Token': token }, body: JSON.stringify({ ip }) }); fetchData(false); } }} className="text-xs bg-blue-50 text-blue-600 px-3 py-1 rounded-lg font-black hover:bg-blue-100 transition-colors">解除</button>
                                      </td>
                                   </tr>
                                );
                             })}
              ...
              <form onSubmit={handleUpdateSettings} className="space-y-6">
                <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl border dark:border-gray-700 shadow-sm space-y-5">
                   ...
                   <div className="flex gap-2">
                      <button type="button" onClick={handleTestEmail} disabled={isTestingEmail} className="flex-1 py-3 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-white rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-gray-200 transition-colors">
                        {isTestingEmail ? <Loader2 className="animate-spin" /> : <Send size={16} />} 接続テスト送信
                      </button>
                   </div>
                </div>
                
                <button type="submit" className="w-full py-4 bg-blue-600 text-white font-black rounded-2xl shadow-xl hover:bg-blue-700 transition-all flex items-center justify-center gap-2">
                   {configMsg ? <CheckCircle2 /> : <Settings />} {configMsg || 'SMTP設定を保存'}
                </button>
              </form>
              ...
              <div className="space-y-6">
                 <div className="bg-indigo-600 p-8 rounded-3xl text-white shadow-xl space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                       ...
                       <button onClick={() => fileInputRef.current?.click()} className="py-3 bg-white text-indigo-600 rounded-xl font-black text-sm flex items-center justify-center gap-2 shadow-lg hover:opacity-90 transition-all"><Upload size={16} /> JSON読み込み</button>
                       <input type="file" accept=".json" ref={fileInputRef} onChange={async (e) => {
                          const file = e.target.files?.[0]; if (!file) return;
                          if (!confirm('データを復元しますか？')) return;
        ...
        {activeTab === 'logs' && (
           <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border dark:border-gray-700 overflow-hidden animate-fade-in">
              <div className="p-6 border-b dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-800/50">
                 ...
                 <button onClick={() => fetchData(false)} className="p-2 text-gray-400 hover:text-blue-500 transition-all"><RefreshCw size={18} className={isLoading ? 'animate-spin' : ''} /></button>
              </div>
              <div className="overflow-x-auto">
                 <table className="w-full text-sm text-left">
                    <thead className="bg-gray-100 dark:bg-gray-900/50 text-[10px] uppercase font-black text-gray-400 tracking-wider">
        ...
        {activeTab === 'messages' && (
           <div className="space-y-4 animate-fade-in">
              {messages.length === 0 ? (
                 <div className="text-center py-24 bg-white dark:bg-gray-800 rounded-3xl border-2 border-dashed dark:border-gray-700 text-gray-400 font-black">メッセージはありません</div>
              ) : (
                 messages.map(m => (
                    <div key={m.id} className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 transition-colors group">
